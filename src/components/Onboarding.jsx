...
export default function Onboarding({ language, setLanguage, setRole, setUserProfile, onComplete }) {
  ...
  const roles = [
    {
      ...
      icon: Users,
      desc: t.citizenDesc,
      borderColor: "border-slate-200 dark:border-slate-800 hover:border-emerald-500",
    ...
    },
    {
      ...
      icon: ShieldAlert,
      desc: t.adminDesc,
      borderColor: "border-slate-200 dark:border-slate-800 hover:border-red-500",
  ...
  const handleLoginSubmit = async (e) => {
    ...
    if (!password) {
      setEmailError(language === "hi" ? "कृपया अपना पासवर्ड दर्ज करें।" : "Please enter your password.");
      return;
    }
    if (!email.toLowerCase().includes("@")) {
      setEmailError(language === "hi" ? "कृपया एक मान्य ईमेल दर्ज करें।" : "Please enter a valid email address.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setEmailError(data.message || "Authentication failed.");
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-radial from-slate-900 via-slate-950 to-black p-4">
      ...
      <div className="max-w-4xl w-full bg-white dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200 dark:border-slate-800 p-8 md:p-12 rounded-3xl shadow-2xl relative overflow-hidden transition-all duration-300">
              ...
              {roles.map((role) => {
                ...
                return (
                  <button
                    ...
                    className={`group text-left p-6 bg-slate-50 dark:bg-slate-950/50 hover:bg-slate-50 dark:bg-slate-950/90 border rounded-2xl transition-all duration-300 shadow-lg hover:shadow-2xl flex flex-col justify-between items-start h-72 cursor-pointer ${role.borderColor} ${role.accentGlow} touch-target`}
                  >
                    <div>
                      <div className={`p-4 rounded-xl border mb-5 transition-transform group-hover:scale-110 duration-300 ${role.iconColor}`}>
                        <IconComponent className="w-6 h-6" />
                      </div>
                      <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 flex flex-col">
                        <span>{role.name}</span>
                        <span className="text-xs text-slate-500 dark:text-slate-500 font-normal mt-0.5">({role.nameHi})</span>
                      </h3>
                      <p className="text-slate-500 dark:text-slate-400 text-xs md:text-sm leading-relaxed mt-2 line-clamp-4">
                        {role.desc}
                      </p>
                    </div>
                    
                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400 group-hover:text-slate-900 dark:text-white transition-colors flex items-center gap-1 mt-4">
        ...
        {step === "gmail_auth" && (
          <div className="max-w-md mx-auto bg-slate-955/40 p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 animate-fadeIn">
            ...
            <div className="flex justify-center mb-6">
              <div className="flex items-center gap-1 bg-white/5 px-4 py-2 rounded-2xl border border-white/10 shadow-inner">
                ...
                <span className="text-slate-500 dark:text-slate-400 text-xs font-mono ml-2 border-l border-slate-200 dark:border-slate-800 pl-2">Sign-In</span>
              </div>
            </div>
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-950 text-xs text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800 rounded-xl pl-10 pr-3 py-3 focus:outline-none focus:border-blue-500"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 text-xs text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800 rounded-xl pl-10 pr-3 py-3 focus:outline-none focus:border-blue-500 font-mono"
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-955 text-xs text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800 rounded-xl pl-10 pr-3 py-3 focus:outline-none focus:border-blue-500 font-mono"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 text-xs text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800 rounded-xl pl-10 pr-10 py-3 focus:outline-none focus:border-blue-500"
                    placeholder="••••••••"
                    required
                    disabled={loading}
                  />
                  <button
                    ...
                    className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-500 dark:text-slate-500 hover:text-slate-900 dark:text-white"
                  >
              ...
              {/* Privacy Warning */}
              <div className="text-[10px] text-slate-500 dark:text-slate-500 leading-normal bg-slate-50 dark:bg-slate-950/50 p-3 rounded-xl border border-slate-300 dark:border-slate-900/60">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-3 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-800 text-slate-350 rounded-xl font-bold text-xs transition-colors cursor-pointer text-center"
                  disabled={loading}
                >
