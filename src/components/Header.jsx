...
import { ShieldAlert, Wifi, WifiOff, Users, Award, Settings, Globe } from "lucide-react";
...
export default function Header({ 
  ...
  return (
    <header className="sticky top-0 z-50 bg-slate-950/85 backdrop-blur-md border-b border-slate-900 py-4 px-4 md:px-8 shadow-md">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xl font-extrabold font-display tracking-wide uppercase text-white">
                {t.appName}
              </span>
              <span className="text-[10px] px-2 py-0.5 rounded bg-slate-900 text-slate-400 font-mono tracking-widest border border-slate-800">
                v1.2
              </span>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          ...
          >
            ...
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-slate-950 text-slate-500 uppercase border border-slate-900/50">
          ...
          <div className="flex items-center gap-1 bg-slate-900/90 p-1 rounded-xl border border-slate-800">
            ...
            <button
              ...
              className={`px-2 py-1 rounded-lg text-[11px] font-extrabold transition-all cursor-pointer touch-target ${
                language === "en" ? "bg-slate-800 text-white border border-slate-700" : "text-slate-500 hover:text-slate-300"
              }`}
            >
              EN
            </button>
            <button
              ...
              className={`px-2 py-1 rounded-lg text-[11px] font-extrabold transition-all cursor-pointer touch-target ${
                language === "hi" ? "bg-slate-800 text-white border border-slate-700" : "text-slate-500 hover:text-slate-300"
              }`}
            >
              हिन्दी
            </button>
          ...
          {userProfile && (
            <div className="flex items-center gap-3 pl-3 border-l border-slate-800">
              <div className="flex flex-col items-end text-right hidden sm:flex">
                <span className="text-xs font-extrabold text-white leading-none font-sans">
                  {userProfile.name}
                </span>
                <span className="text-[9px] text-slate-500 font-mono select-all mt-0.5">
                  {userProfile.email}
                </span>
              </div>
              <div 
                className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs text-white shadow-md border border-white/20 select-none bg-gradient-to-br ${meta.avatarGradient}`}
              ...
              <button
                onClick={onSignOut}
                className="text-[10px] text-slate-400 hover:text-red-400 font-bold bg-slate-900 hover:bg-slate-950 px-2 py-1.5 rounded-lg border border-slate-800 hover:border-red-900 transition-all cursor-pointer font-sans"
              >
